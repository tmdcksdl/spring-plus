...
public interface TodoRepository extends JpaRepository<Todo, Long> {
    ...
    @Query("SELECT t FROM Todo t WHERE t.modifiedAt >= :startDateTime ORDER BY t.modifiedAt DESC")
    Page<Todo> findAllByModifiedAtAfter(@Param("startDateTime") LocalDateTime startDateTime, Pageable pageable);

    @Query("SELECT t FROM Todo t WHERE t.modifiedAt <= :endDateTime ORDER BY t.modifiedAt DESC")
